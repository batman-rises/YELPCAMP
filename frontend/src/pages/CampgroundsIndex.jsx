...
export default function CampgroundsIndex() {
  ...
  const fetchCampgrounds = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("/campgrounds", {
        params: { search, minPrice, maxPrice, sort, rating },
      });
  ...
  return (
    <div className="page-container py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Discover India's finest camping spots
          </p>
        </div>
        {currentUser && (
          <Link
      ...
      <div className="bg-white border border-[#e3dfd7] rounded-2xl p-4 mb-6 shadow-soft">
        <div className="flex gap-3 flex-wrap items-end">
          ...
          >
            ...
            Filters
            {hasFilters && (
              <span className="bg-forest-600 text-white text-xs w-4 h-4 rounded-full flex items-center justify-center">
                !
              </span>
            )}
          </button>
          ...
          {hasFilters && (
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-red-500 px-3 py-3 transition-colors"
            >
              <X size={14} />
              Clear
            </button>
      ...
      {/* Main content — list left, map right sticky */}
      <div className="flex gap-6 items-start">
        {/* Left — campground cards */}
        <div className="flex-1 min-w-0">
          {!loading && (
            <p className="text-sm text-gray-500 mb-5">
              {campgrounds.length} campground
              {campgrounds.length !== 1 ? "s" : ""} found
              {search && (
                <span>
                  {" "}
        ...
        <div className="hidden lg:block w-[420px] xl:w-[480px] shrink-0 sticky top-24">
            ...
            {campground.avgRating ? (
              <>
                ...
                <span className="text-sm font-medium text-gray-700">
                  {campground.avgRating.toFixed(1)}
                </span>
