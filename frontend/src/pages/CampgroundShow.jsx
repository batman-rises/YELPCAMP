...
export default function CampgroundShow() {
  ...
  const isDateRangeAvailable = (cin, cout) => {
    if (!campground?.bookedDates?.length) return true
    const start = new Date(cin)
    const end = new Date(cout)
    return !campground.bookedDates.some(bd => {
  ...
  const isDatesConflicting = checkIn && checkOut && !isDateRangeAvailable(checkIn, checkOut)
      ...
      <div className="grid lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 flex flex-col gap-6">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400">
            <Link to="/campgrounds" className="hover:text-forest-600 transition-colors">Campgrounds</Link>
            <span>/</span>
          ...
          <div>
            <div className="flex items-start justify-between gap-4">
              <h1 className="font-display text-3xl text-forest-900">{campground.title}</h1>
            ...
            <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-gray-500">
              <span className="flex items-center gap-1.5"><MapPin size={14} />{campground.location}</span>
          ...
          <div className="bg-white rounded-2xl border border-[#e3dfd7] p-6">
            <h2 className="font-display text-lg text-forest-800 mb-3">About this campground</h2>
            <p className="text-gray-600 leading-relaxed">{campground.description}</p>
          </div>

          {/* Author actions */}
          {isAuthor && (
            <div className="flex items-center gap-3">
              <Link to={`/campgrounds/${id}/edit`} className="btn-outline flex items-center gap-1.5">
                <Edit size={14} />
                Edit
              </Link>
              <button
          ...
          <ReviewSection
            ...
            onReviewChange={() => {
              api.get(`/campgrounds/${id}`).then(res => setCampground(res.data.campground))
            }}
          />
        </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-[#e3dfd7] p-6 shadow-card sticky top-24">
            ...
            <button
              ...
              disabled={booking || isDatesConflicting}
            ...
            {isDatesConflicting && (
              <p className="text-xs text-red-500 text-center mt-1 flex items-center justify-center gap-1">
                <span>These dates are already booked. Please choose different dates.</span>
              </p>
            )}
          ...
          {/* Map */}
          <div>
            <h3 className="font-display text-lg text-forest-800 mb-3">Location</h3>
            <ShowMap campground={campground} />
            <p className="text-sm text-gray-500 mt-2 flex items-center gap-1.5">
              <MapPin size={13} />{campground.location}
            </p>
          </div>
        </div>
      </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-lifted">
            ...
            <h3 className="font-display text-xl text-gray-900 mb-2">Delete Campground?</h3>
            <p className="text-gray-500 text-sm mb-6">This will permanently remove the campground and all its reviews. This cannot be undone.</p>
